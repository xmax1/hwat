...
class Ansatz_fb(nn.Module):
	...
	def forward(ii, r: torch.Tensor) -> torch.Tensor:
		...
		rr_len = torch.linalg.norm(rr + eye, dim=-1, keepdim=True) 
		...
		for l_i, (V, W) in enumerate(zip(ii.s_lay, ii.p_lay)):
			...
			s_v = torch.tanh(V(s_v_block)) + (0.0 if not l_i else s_v)
			p_v = torch.tanh(W(p_v)) + (0.0 if not l_i else p_v)
		...
		ones = torch.ones((n_b, ii.n_det,), device=device, dtype=dtype)
		zeros = torch.zeros((n_b, ii.n_det,), device=device, dtype=dtype)
		...
		maxlogdet, idx = torch.max(logdet, dim=-1, keepdim=True) # (n_b, 1), (n_b, 1)
		if ii.n_det < 2:
			maxlogdet = torch.zeros_like(maxlogdet)
