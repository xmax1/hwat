...
# When using --cpus-per-task to run multithreaded tasks, be aware that CPU binding is inherited from the parent of the process. This means that the multithreaded task should either specify or clear the CPU binding itself to avoid having all threads of the multithreaded task use the same mask/CPU as the parent. Alternatively, fat masks (masks which specify more than one allowed CPU) could be used for the tasks in order to provide multiple CPUs for the multithreaded tasks.
	...
	class dist(Sub):
		exchange_dir        = property(lambda _: _._p.exp_path / 'exchange')
		accumulate_step     = 5
		_dist_id: str       = ''
		head: bool          = True
		gpu_id: str         = property(lambda _: \
			''.join(run_cmds('nvidia-smi --query-gpu=pci.bus_id --format=csv,noheader')[0]).split('.')[0]
		)
		sync: list          = ['params', ]
	...
	hostname: str       = property(lambda _: run_cmds('hostname')[0])
	...
	_ignore:            list     = ['d', 'cmd', 'partial', 'lo_ve', 'log', 'merge', 'accumulate']
...
# )[ii.exe_mode]
		...
		cmd = f'python {ii.run_name} {ii.cmd}'
		for i in range(ii.n_gpu):
			...
			s += [f'srun --gpus=1 --cpus-per-task=4 --mem-per-cpu=1024 --ntasks=1 --exclusive --label \
					{cmd} --head {head} > {device_log_path} 2>&1 & \n' ]
		s += ['wait \n']
	...
	def accumulate(ii, step: int, v_tr:dict):
		...
		for k,v in v_tr.copy().items():
			if np.isscalar(v):
				continue
			v_path = (ii.dist.exchange_dir / f'{k}_{step}_{ii.dist._dist_id}').with_suffix('.pk')
			...
			if ii.dist.head:
				...
				while n_ready < ii.n_gpu:
					k_path_all = list(ii.dist.exchange_dir.glob(f'{k}_{step}*')) 
				...
				for p in k_path_all:
					...
					leaves, tree_spec = optree.tree_flatten(v_dist_i)
					leaves = [v[None] for v in leaves]
					
					if l_all is None:
						l_all = leaves
					else: 
						l_all = [np.concatenate([v_all, v], axis=0) for v_all, v in zip(l_all, leaves)] 

				[p.unlink() for p in k_path_all]
    
				tree_flat_mean = [v.mean(0) for v in l_all]
				v_tr[k] = optree.tree_unflatten(treespec=tree_spec, leaves=tree_flat_mean)
	
				if k in ii.dist.sync:
					[ii.lo_ve(path=add_to_Path(p, '-mean').with_suffix('.pk'), data=v_tr[k]) for p in k_path_all]
					v_path_mean.unlink()
			else:
				if k in ii.dist.sync:
					while not v_path_mean.exists():
						sleep(0.01)
					v_tr[k] = ii.lo_ve(path=v_path_mean)
					v_path_mean.unlink()
